...
public class Code_Review_2_Nootnoot {
	...
	public static final String open_weather_api_key = "b369057d518ed3e182c04c76c1ec73fe"; // This is the OpenWeatherMap
																							// API key
	...
	public static void main(String[] args) throws Exception {
		...
		// This piece of code is an additional feature added to Noonoot which lets it
		// communicate with user with voice
		// Created a voice object
		...
		while (true) {
			...
			// Calling plan function that takes care of the user requirement when user
			// inputs 5 destinations
			if (city.equalsIgnoreCase("plan")) {
				plan();
				continue;
			}

			// Calling the function which helps the user by showing all the possible chatbot
			// commands
			...
			// Added a validation to ensure that the temperature value is not empty before
			// parsing it into a double
			...
			// Calling the weatherCondition method and also saving its results in a variable
			// of string.
			String weatherCondition = getWeatherCondition(Double.parseDouble(temperature));

			// Calling the clothingSuggestion method and saving its results in a string
			// variable.
	...
	// This method prints a message and simulates processing time by pausing
	...
	public static void delayedExecution() throws InterruptedException {
		...
		// From this feature creates a more realistic experience for the user by
		// introducing a delay that simulates actual processing time
	...
	// This code functionality lets the user too quit the Nootnoot chatbot via
	// "quit" or "q" commands
	...
	// Code or method, that suggest the type of cloth to wear while going outside,
	// according to the temprature of the area.
	...
	// Method used to express the type of weather/ weather ccondition, matching with
	// the provided location and temprature.
	...
	// This method fetches weather data from the OpenWeatherMap.org API website and
	// returns the weather date in String format
	...
	public static String fetchWeatherData(String city, String open_weather_api_key) {
		...
		// Implementing try-catch block code for handling any kind of errors that occur
		// during I/O operations
		try {
			URL url = new URL(
					"http://api.openweathermap.org/data/2.5/weather?q=" + city + "&appid=" + open_weather_api_key);
			HttpURLConnection connection = (HttpURLConnection) url.openConnection(); // created a new connection to the
																						// API using HTTP protocol
			connection.setRequestMethod("GET"); // setting request method as 'GET'
			BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream())); // Processes
																											// the
																											// response
																											// from the
																											// website
		...
		// This block of code handles any exceptions that occur while trying to fetch
		// weather data from OpenWeatherMap.org api website
		catch (Exception e) {
	...
	// Now the extracted temperature value is converted into String format with 2
	// decimal places.
	public static String extractTemperatureInCelsius(String weatherData) {
		...
		double celsiusTemperature = kelvinTemperature - 273.15; // Converting Kelvin to Celsius, because Celsius scale
																// is commonly used
	...
	// This method is a plan-trip planner which provides clothing requirements based
	// on the weather conditions in all 5 locations
